# Hermes Integration Guide: Linear

> **Status:** Implemented. The README.md and setup.sh have been extracted into
> `.hermes/integrations/linear/`. This document remains as a design reference
> covering MCP server details, auth rationale, environment variables, and
> testing checklists.

## Goal

Add a Linear integration to the Hermes template so that Hermes can interact with Linear (project management — issues, projects, cycles, teams) via Linear's MCP server.
...
3. **Closest reference implementation:** `.hermes/integrations/atlassian/` — remote MCP server, browser-based auth
...
2. Click **Create Key**
3. Give it a descriptive name (e.g., `Hermes`)
...
- **Work alongside Linear's Slack integration** — Hermes creates/updates in Linear; Linear's own Slack app handles notifications automatically. No extra wiring needed.
...
- [ ] Root `README.md` — Add row: `| [Linear](.hermes/integrations/linear/) | Issues, projects, cycles | /help then follow prompts |`
...
- **Linear's Slack integration is separate.** When Hermes creates or updates issues in Linear, Linear's own Slack app sends notifications. No extra configuration needed on Hermes's side.
...
- [ ] Hermes confirms before create/update/comment actions (Danger Zone compliance)
